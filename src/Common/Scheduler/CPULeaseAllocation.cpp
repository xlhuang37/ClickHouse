...
bool CPULeaseAllocation::schedule(std::unique_lock<std::mutex> &)
{
    ...
    ///
    /// All queries share fairly across the elastic bands. The band is picked from
    /// `requested_ns` (cumulative consumed + outstanding granted quantum budget), so a
    /// query that has used less CPU sits in a higher (lower-index) band. Compared with
    /// the previous continuous `consumed_ns / 1024^3` priority, the discrete bands bound
    /// the number of buckets and avoid the "same age preempt each other" thrash when two
    /// queries have near-identical virtual time.
    Priority priority{};
