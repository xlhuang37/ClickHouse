...
bool CPULeaseAllocation::renew(Lease & lease)
{
    ...
    if (realtime_slot_id.has_value() && *realtime_slot_id == lease.slot_id)
    {
        ...
        // so it is not added to consumed_ns/requested_ns and does not affect CPU-lease accounting;
        // and it is never preempted. It still honors shutdown.
        report_span.reset();
        if (shutdown)
        {
            disableRealtime(lease.slot_id);
...
size_t CPULeaseAllocation::computeCap() const
{
    ...
    /// The hard cap is `max_threads`. If the pipeline exposes its number of ready tasks, we
    /// additionally clamp to `max(min(max_threads, ready_tasks + running_count), 2)` to avoid
    /// over-provisioning CPU quanta for queries that cannot keep `max_threads` threads busy (e.g.
    /// blocked by a pipeline breaker). Rationale for each term:
    ///  - `running_count` keeps enough in-flight quanta to cover every currently running thread
    ///    so consumption does not starve them;
    ///  - `ready_tasks` adds headroom proportional to the amount of parallelizable work available;
    ///  - the `max(..., 2)` floor guarantees progress at construction time (pipeline queues are
...
bool CPULeaseAllocation::schedule(std::unique_lock<std::mutex> &)
{
    ...
    requested_ns += cost;
    const auto enqueue_result = requests.enqueue(cost, requested_ns, cap < allocated);
