 ...
 *
 * All levels 0..(kPriorityLevels-1) are elastic bands; the caller decides which
 * band by passing a `Priority{value}` whose `value` is the bucket index. Any
 * out-of-range value is clamped into [0, kPriorityLevels-1].
 *
...
{
public:
    /// Number of priority levels. Level 0 = highest priority; level K-1 = lowest.
