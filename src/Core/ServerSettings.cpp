...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **See Also**
    ...
    DECLARE(UInt64, cpu_slot_mlfq_layer_width, 4, R"(
    Topology of the CPU Multi-Level Feedback Queue (MLFQ): number of priority levels (CPU consumption bands) per parallelism layer. This setting takes effect only at server startup and requires a restart to change. Makes sense only when `cpu_slot_preemption` is enabled.
    )", 0) \
    DECLARE(UInt64, cpu_slot_mlfq_num_layers, 2, R"(
    Topology of the CPU Multi-Level Feedback Queue (MLFQ): number of parallelism layers. A query with fewer allocated slots sits in a lower (higher-priority) layer. This setting takes effect only at server startup and requires a restart to change. Makes sense only when `cpu_slot_preemption` is enabled.
    )", 0) \
    DECLARE(UInt64, cpu_slot_mlfq_leveling_threads, 8, R"(
    Topology of the CPU Multi-Level Feedback Queue (MLFQ): number of allocated CPU slots that map to one parallelism layer. This setting takes effect only at server startup and requires a restart to change. Makes sense only when `cpu_slot_preemption` is enabled.
    )", 0) \
