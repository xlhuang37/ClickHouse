...
The policy on how to perform a scheduling of CPU slots specified by `concurrent_threads_soft_limit_num` and `concurrent_threads_soft_limit_ratio_to_cores`. Algorithm used to govern how limited number of CPU slots are distributed among concurrent queries. Scheduler may be changed at runtime without server restart.
    ...
    **See Also**
    ...
    DECLARE(UInt64, max_realtime_threads, 8, R"(
    The number of "real-time thread" slots available process-wide. When a CPU-lease query is in its inelastic phase (small number of ready tasks), its master thread may take one of these slots and switch to Linux real-time scheduling (`SCHED_FIFO`) to monopolize a CPU core, returning the slot when it leaves the inelastic phase or finishes. Set to 0 to disable. Requires the `CAP_SYS_NICE` capability; without it, the transition is silently skipped.

    **Example**

    ```xml
    <max_realtime_threads>8</max_realtime_threads>
    ```
    )", 0) \
    DECLARE(UInt64, realtime_thread_priority, 1, R"(
    The `SCHED_FIFO` priority used when a query master thread transitions to Linux real-time scheduling (see `max_realtime_threads`). A modest value is recommended so that real-time query threads do not starve essential kernel threads.

    **Example**

    ```xml
    <realtime_thread_priority>1</realtime_thread_priority>
    ```
    )", 0) \
